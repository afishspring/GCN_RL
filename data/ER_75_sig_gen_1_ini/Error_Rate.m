...
for dd=100:100:900
    ...
    myfilename = sprintf('Sstate_%d_%d.mat',iiii, dd);
...
title('ER graph with \gamma = 0.0075 and one inital infected node')
...
for i=1:iter
   ...
   if(ECLow_noise(2,d) >EngLR_s(i,d) && EngLR_s(i,d) > ECLow_noise(1,d))
       ErrorELow(d)=(ErrorELow(d)+1)/iter;
   end
    end
end
...
plot(X,Y)
xlabel('Number of infected nodes')
ylabel('Probability of detection using the  Energy CL')
title('ER graph with \gamma = 0.0075 and one inital infected node')
...
plot(X,Y)
xlabel('Number of infected nodes')
ylabel('Probability of detection using the  Energy CH')
title('ER graph with \gamma = 0.0075 and one inital infected node')
